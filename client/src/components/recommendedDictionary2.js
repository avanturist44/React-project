...
function RecommendedDictionary2() {
  ...
  async function ShowWords() {
    ...
    try {
      fetch(`http://localhost:8800/dictionary/vocabulary/2`, {
        method: "GET",
        headers: {
  ...
  async function AddWords() {
    const token = localStorage.getItem('token')
    try {
      fetch('http://localhost:8800/dictionary/vocabulary/2/add', {
        method: "POST",
        body: JSON.stringify({
          data: data
        }),
  ...
  async function DeleteWord(wordToDelete) {
    const token = localStorage.getItem('token')
    try {
      await fetch("http://localhost:8800/dictionary/vocabulary/2/delete", {
        method: "POST",
        body: JSON.stringify({
          word: wordToDelete
        }),
...
export default RecommendedDictionary2