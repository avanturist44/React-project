...
function RecommendedDictionary1() {
  ...
  async function ShowWords() {
    ...
    try {
      fetch(`http://localhost:8800/dictionary/vocabulary/1`, {
        method: "GET",
        headers: {
  ...
  async function AddWords() {
    const token = localStorage.getItem('token')
    try {
      fetch('http://localhost:8800/dictionary/vocabulary/1/add', {
        method: "POST",
        body: JSON.stringify({
          data: data
        }),
  ...
  async function DeleteWord(wordToDelete) {
    const token = localStorage.getItem('token')
    try {
      await fetch("http://localhost:8800/dictionary/vocabulary/1/delete", {
        method: "POST",
        body: JSON.stringify({
          word: wordToDelete
        }),
...
export default RecommendedDictionary1